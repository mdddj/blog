...
export default function Page() {
    const {data,loading,error} = useRequest<ApiResponse<FriendLink[]>>(()=>fetchData())
            ...
            }
            {
                data && <div className={'grid grid-cols-3 gap-2'}>
                        ...
                        data.data.map(value => {
                            return <div key={value.id}>
                                <User name={value.name} avatarProps={{
                                    src: value.logo
                                }} description={<Link href={value.url}  isExternal={true} size={'sm'}>{value.intro}</Link>} />
                            </div>
                        })
                    }
...

                    {
                        ...
                        isEmpty && <span className={'text-default-500 text-sm'}>暂无友链</span>
                    }
                </div>
            }
