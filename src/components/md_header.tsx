...
const Documents: React.FC<Props> = ({ md }) => {
  ...
  const formatNavItem = (headerDom: NodeListOf<HTMLElement>) => {
     ...
     */
    let result = Array.prototype.slice.call(headerDom).map((item, index) => {
      return {
        href: "#" + index,
        key: "" + index,
  ...
  return (
    <motion.div
      className={"card rounded m-2 hidden md:block"}
    ...
    >
      <div className={"card-body"}>
        {titles.length > 0 && (
          <h2 className={"font-bold text-2xl card-title"}>目录</h2>
        )}
        <aside>
          {titles.length > 0 && (
            <Anchor
